...
namespace Microsoft.AspNetCore.Hosting
{
    ...
    {
        static void WriteLine(this IWebHost host, string value)
        {
            if (host is AndroidWebHost host2)
            {
                host2.WriteLine(value);
            }
            else
            {
        ...
        public static async Task RunCompatAsync(this IWebHost host, CancellationToken token = default)
        {
            ...
            using (var cts = new CancellationTokenSource())
            {
                ...
                AttachCtrlcSigtermShutdown(host, cts, done, shutdownMessage: shutdownMessage);
        ...
        private static async Task RunAsync(this IWebHost host, CancellationToken token, string shutdownMessage)
        {
            using (host)
            {
                ...
                {
                    host.WriteLine($"Hosting environment: {hostingEnvironment.EnvironmentName}");
                    host.WriteLine($"Content root path: {hostingEnvironment.ContentRootPath}");
                    ...
                    if (serverAddresses != null)
                    {
                        ...
                        {
                            host.WriteLine($"Now listening on: {address}");
                        }
                    }

                    if (!string.IsNullOrEmpty(shutdownMessage))
                    {
                        host.WriteLine(shutdownMessage);
                    }
                }
        ...
        private static void AttachCtrlcSigtermShutdown(IWebHost host, CancellationTokenSource cts, ManualResetEventSlim resetEvent, string shutdownMessage)
        {
            void Shutdown()
            {
